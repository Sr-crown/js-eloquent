...
const codigoChar = (code_Char)=>{
    for ( codigo of SCRIPTS){
        if (codigo.ranges.some(([desde,hasta])=>{
            return code_Char >= desde && code_Char < hasta;
        })){
            return codigo;
        }
...
const contarPor = (elementos,nombre_grupo)=>{
    let cuentas =[];
    for(let elemento of elementos){
        let nombre = nombre_grupo(elemento);
...
const direcLenguaje = (texto)=>{
  let codigos = contarPor(texto,caracter =>{
    let codigo = codigoChar(caracter.codePointAt(0));
    return codigo ? codigo.direction : "not-found"; 
  }).filter(({nombre}) => nombre != "not-found");

  let seguro = codigos.reduce((n,{cuenta})=>n+cuenta,0)
  if (seguro == 0){
    return "No se encontraron similitudes"
  }
  return codigos.map(({nombre,cuenta})=>{
    return 'el sentido de la escritura es: '+nombre;
  })
}

console.log(direcLenguaje("hola"));